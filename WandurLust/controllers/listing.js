...
module.exports.search = async (req, res) => {
  try {
    const { q, category, minPrice, maxPrice, country, placementLevel } =
      req.query;
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 9;
    const skip = (page - 1) * limit;

    // If no search query, redirect to main listings
    if (!q || q.trim() === "") {
    ...
    res.render("listings/search.ejs", {
      ...
      limit,
      searchQuery: q || "",
      filters: { category, minPrice, maxPrice, country, placementLevel },
    });
  } catch (error) {
